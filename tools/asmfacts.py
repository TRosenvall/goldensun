...
def functions(path, include_not_c=False):
    """Names of the hand-written functions defined in a .s.

    Functions listed in tools/not_c.py are EXCLUDED by default: they are the
    MP2K sound driver, which is hand-written assembly rather than compiler
    output, so they are already in their final form and are not elevation
    candidates. Pass include_not_c=True when you want the raw list -- the
    orphaned-reference check does, because those symbols are still linked.
    """
    txt = open(path, errors="replace").read()
    if "Generated by gcc" in txt.split("\n")[0]:
        return []
    if not include_not_c:
        try:
            from not_c import is_not_c
        except ImportError:
            from tools.not_c import is_not_c
        if is_not_c(path):
            return []
...
def carries_data(path):
    ...
    Three signals, and any is enough:

      * an .incbin / .incrom / .incdata blob -- C cannot express one at all;
      * an explicit .section .rodata or .data;
      * a label the functions in the file never branch to, which is a data
        label rather than control flow.

    THE DIRECTIVE LIST IS NOT COSMETIC. This checked only `.incbin` until batch
    60, when a .s holding one function plus a `.section .rodata` with an
    `.incrom` blob was reported as "no data, convert it directly". The .s was
    deleted, and the link failed on the exported label the blob carried. The
    same class of mistake broke the build once before with a `.data` section --
    that time the directive WAS .incbin and the check simply did not exist yet.
    ...
    if "Generated by gcc" in txt.split("\n")[0]:
        return False
    if re.search(r"^\s*\.inc(bin|rom|data)\b", txt, re.M):
        return True
    if re.search(r"^\s*\.section\s+\.(ro)?data\b", txt, re.M):
        return True
...
def main():
    ...
    for p in sys.argv[1:]:
        ...
        fns = functions(rel, include_not_c=True)
