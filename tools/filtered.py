...
def generated(path, lines=None):
    """True if this .s is gcc's OWN OUTPUT rather than disassembly to elevate.

    The obvious test -- `".gcc2_compiled." in some line` -- is a SUBSTRING
    search, and it is wrong. Hand-written disassembly discusses that string in
    its `@` prose ("marking closure with .gcc2_compiled., releasing tiles
    with ..."), so the substring test rejects genuine targets.

    Measured when this was written: 72 tracked, hand-written .s files carrying
    126 functions matched it. templated.py and census.py both skipped those
    files outright, so 126 functions had NEVER BEEN RANKED as candidates; and
    split_s.py used it to label a clash destination "GENERATED from a .c",
    which tells the reader the file is the build input for something already
    elevated -- the opposite of the truth, at a prompt where they then decide
    what to overwrite.

    gcc identifies itself two ways that prose cannot imitate: the banner on
    LINE 1, and `.gcc2_compiled.:` as a LABEL at the start of a line. Test
    those, not the bare substring.
    """
    if lines is None:
        lines = open(path, errors="ignore").readlines()
    if lines and "Generated by gcc" in lines[0]:
        return True
    return any(l.startswith(".gcc2_compiled.:") for l in lines[:40])
