/* OvlFunc_910_200850c  --  0x0200850c
 *
 * Four independent arrival fixups, each gated on its own flag.
 *
 * EIGHT LEVERED CONSTANTS, which is the most in one function so far, and they
 * are declared together at the top because every use is inside one of the four
 * `if` bodies -- the tests themselves are the boundaries. Note `p1` and `p2`
 * are the SAME value (0x1070000) passed to two different calls and each gets
 * its own local: the ROM rebuilds it, so it is REBUILT, one per site.
 */
