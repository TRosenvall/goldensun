/* OvlFunc_957_200834c -- FindPushableFacingPlayer, overlay rom_7e3e08, ovl_30_a_a_a_c_c_a.
 *
 ...
 *
 *   OK OvlFunc_957_200834c -- 296 bytes, 143 encodings and 5 relocations identical
 ...
 *   overlays/rom_7e3e08/overlay.ld:22
 *       asm/overlays/rom_7e3e08/ovl_30_a_a_a_c_c_a.o(.text)
 * The `.L` table symbols reached through __asm__ labels below are defined in
 * asm/overlays/rom_7e3e08/ovl_30_c_c_c_c_b_c.s and are ALREADY `.global`
...
extern int gStep_957__0200beb4[] __asm__(".L3eb4");
extern int gModelId_957__0200bef4[] __asm__(".L3ef4");
extern struct Rect gBox_957__0200bf0c[] __asm__(".L3f0c");
extern struct Ent *__MapActor_GetActor(int slot);

struct Ent *OvlFunc_957_200834c(int *facingOut, int *slotOut, int *modelOut)
{
    struct Ent **tbl;
    struct Ent *pl;
    struct Ent *e;
    ...
    *facingOut = pl->facing >> 12;
    for (slot = 8; slot <= 0x41; slot++) {
        e = tbl[slot];
        for (i = 0; i <= 5; i++) {
            if (*e->f50->f28 != gModelId_957__0200bef4[i])
                continue;
            *modelOut = i;
            tx = ((pl->x >> 16) + (gStep_957__0200beb4[*facingOut] >> 16)) >> 4;
            tz = ((pl->z >> 16) + (short)gStep_957__0200beb4[*facingOut]) >> 4;
            ex = *(short *)((char *)e + 0xa);
            x0 = (ex + gBox_957__0200bf0c[i].x0) >> 4;
            ez = *(short *)((char *)e + 0x12);
            z0 = (ez + gBox_957__0200bf0c[i].z0) >> 4;
            x1 = (ex + gBox_957__0200bf0c[i].x1) >> 4;
            z1 = (ez + gBox_957__0200bf0c[i].z1) >> 4;
            if (x0 > tx)
                continue;
            if (tx >= x1)
                continue;
