/* OvlFunc_959_200834c -- FindPushableFacingPlayer, overlay rom_7e7574, ovl_30_c_c_a.
 *
 ...
 *
 *   OK OvlFunc_959_200834c -- 296 bytes, 143 encodings and 5 relocations identical
 ...
 *   overlays/rom_7e7574/overlay.ld:22
 *       asm/overlays/rom_7e7574/ovl_30_c_c_a.o(.text)
 * The `.L` table symbols reached through __asm__ labels below are defined in
 * asm/overlays/rom_7e7574/ovl_30_c_c_c_c.s and are ALREADY `.global`
...
extern int gStep_959__0200ded8[] __asm__(".L5ed8");
extern int gModelId_959__0200df18[] __asm__(".L5f18");
extern struct Rect gBox_959__0200df30[] __asm__(".L5f30");
extern struct Ent *__MapActor_GetActor(int slot);

struct Ent *OvlFunc_959_200834c(int *facingOut, int *slotOut, int *modelOut)
{
    struct Ent **tbl;
    struct Ent *pl;
    struct Ent *e;
    ...
    *facingOut = pl->facing >> 12;
    for (slot = 8; slot <= 0x41; slot++) {
        e = tbl[slot];
        for (i = 0; i <= 5; i++) {
            if (*e->f50->f28 != gModelId_959__0200df18[i])
                continue;
            *modelOut = i;
            tx = ((pl->x >> 16) + (gStep_959__0200ded8[*facingOut] >> 16)) >> 4;
            tz = ((pl->z >> 16) + (short)gStep_959__0200ded8[*facingOut]) >> 4;
            ex = *(short *)((char *)e + 0xa);
            x0 = (ex + gBox_959__0200df30[i].x0) >> 4;
            ez = *(short *)((char *)e + 0x12);
            z0 = (ez + gBox_959__0200df30[i].z0) >> 4;
            x1 = (ex + gBox_959__0200df30[i].x1) >> 4;
            z1 = (ez + gBox_959__0200df30[i].z1) >> 4;
            if (x0 > tx)
                continue;
            if (tx >= x1)
                continue;
