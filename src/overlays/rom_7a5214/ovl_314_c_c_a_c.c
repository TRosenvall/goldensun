/* WHOLE-FILE CONVERSION of asm/overlays/rom_7a5214/ovl_314_c_c_a_c.s --
 * OvlFunc_918_2009004 is its only function and datacheck.py reports no data
 * sections, so no split and no linker change.  213 instructions, 544 bytes, 220
 * encodings and 40 relocations identical.
 *
 * `_AREA_2d` is already in area.sym:237; this file uses the plain
 * `extern int _AREA_2d;` form that landed files use.  No shim -- a verification
 * shim belongs in the scratch candidate only.
 *
 * ================================================================
 * `do { } while (0)` IN A MACRO BODY PLANTS TWO TOTAL SCHEDULING BARRIERS, AND
 * THAT IS WHY THIS FUNCTION WOULD NOT CLOSE
 * ================================================================
 *
 * Confirmed at haifa-sched.c:3714 onward.  A NOTE_INSN_LOOP_BEG / LOOP_END sets
 * `schedule_barrier_found`, which makes the next real insn depend on EVERY PRIOR
 * USE, SET AND CLOBBER OF EVERY REGISTER and calls `flush_pending_lists`.
 * `do { } while (0)` emits exactly those notes.  Plain `{ ... }` emits only
 * NOTE_INSN_BLOCK_BEG/END, which are NOT in that list -- the same file excludes
 * loop notes from the ordinary note list for precisely this reason.
 *
 * So the CALL_VIA_R4 macro's conventional `do { } while (0)` wrapper was an
 * UNINTENDED total barrier around its own body.  The 0xa0 << 16 addend build had
 * been combined into the macro's `_a = a0` copy -- visible in .19.flow2 as insns
 * 603/604 sitting AFTER `note 161 NOTE_INSN_LOOP_BEG` -- so it could never be
 * hoisted into the ROM's interleave.  Replacing the wrapper with `{ ... }`, with
 * `if (1) { ... } else (void)0`, or with no braces and function-scope temporaries
 * all work identically.
 *
 * THIS ALSO EXPLAINS THE TREE'S DELIBERATE `do { } while (0);` BARRIER DEVICE,
 * which 52 landed files use and which has until now been an observation rather
 * than a mechanism.  The corollary is the part to carry: WHEN A MACRO BODY NEEDS
 * TO SCHEDULE WITH ITS SURROUNDINGS, DO NOT WRAP IT IN do-while.
 *
 * The second edit was removing a deliberate `do { } while (0);` that stood between
 * `w = base + (0x82 << 1);` and `fv = Func_8000888;` -- same mechanism, right tool,
 * wrong place.
 *
 * A 90-VARIANT SWEEP GIVES 54 EXACT, so the requirement is exactly two things: no
 * do-while in the macro wrapper, and no barrier immediately before `fv =`.
 * Barriers earlier in that region are free, and `a0` / `w` may each be spelled
 * three ways.  A greedy ladder then found two pins inert SINGLY AND JOINTLY -- the
 * PIN1 on the second __GetFlag(0x109) and the PIN1 on the second
 * __MapActor_Surprise -- both dropped, still exact.
 *
 * ================================================================
 * THE PARK'S DIAGNOSIS WAS WRONG, AND THAT IS WORTH MORE THAN THE LANDING
 * ================================================================
 *
 * src/non_matching/ovl_7a5214/2009004.c called the residue "A SINGLE
 * FOUR-INSTRUCTION sched2 INTERLEAVE".  With -fno-schedule-insns2 the park's
 * output is INSTRUCTION-FOR-INSTRUCTION IDENTICAL in that region -- so sched2 was
 * doing NOTHING there.  The ROM's order is the sched2 fill; the park's was a
 * BARRIERED NON-SCHEDULE.  It was also 5 positions, not 4.
 *
 * THE GENERAL TRAP: a residue that looks like a scheduling difference can be the
 * ABSENCE of scheduling.  The one-line test is -fno-schedule-insns2 -- if it
 * changes nothing in the window, sched2 is not the culprit and something is
 * barriering it.
 *
 * The park's `.call_via r4` clobber-list correction held and is carried here: `lr`
 * SURVIVES because `mov r12, pc / bx r4` puts the return address in r12, while r2
 * and r3 ARE clobbered and omitting them is a real miscompile.
 *
 * No per-file Makefile flag override applies to this stem -- rom_7a5214's only
 * per-file rule is ALIAS_CFLAGS for ovl_314_c_c_c_b at Makefile:737.
 */
