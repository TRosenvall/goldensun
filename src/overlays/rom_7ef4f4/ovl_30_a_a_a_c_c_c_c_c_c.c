/* OvlFunc_965_2008ae8 -- MATCHES on the default flags.
 * ref: asm/overlays/rom_7ef4f4/ovl_30_a_a_a_c_c_c_c_c_c.s
 ...
 * One of three byte-identical copies -- see src/rom_8a000/rom_9a44c_a_a_a_c.c
 * and src/overlays/rom_7f2f14/ovl_30_a_a_a_c_a_c_c.c.  They differ only in symbol names.
 *
 * THREE LEVERS, in the order they were found.
 *
 * 1. TWO `__CreateActor` CALL SITES, not one with a chosen id.  The ROM's
...
extern void OvlFunc_965_2008ab0(void);
extern struct Script *L3134[] __asm__(".L3134");

void OvlFunc_965_2008ae8(int x, int y, int z, int a, int b, int c,
                        unsigned int flags, struct Launch *p)
{
    ...
    caster = __MapActor_GetActor(0);
    if ((flags & 0x100000) && p != 0)
        n = __CreateActor(p->f18, x, y, z);
    else
        n = __CreateActor(0xde, x, y, z);
    if (n != 0) {
        s = n->part;
        __Actor_SetAnim(n, (flags + 1) & 0xf);
        __Actor_SetScript(n, (unsigned char *)L3134[flags & 0xf]);
        n->f55 = 0;
        s->f26 = 0;
        n->f6c = OvlFunc_965_2008ab0;
        n->f44 = a;
        n->f48 = b;
        n->f4c = c;
