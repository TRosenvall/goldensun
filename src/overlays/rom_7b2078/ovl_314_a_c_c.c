/* Cluster OvlFunc_926_200834c..OvlFunc_926_200834c extracted from goldensun/asm/overlays/rom_7b2078/ovl_314_a_c_c.s.
 *
 * The .s held ONLY this function and no data, so no split was needed.
 *
 * GetEntrances, two-way form. This one returns a NAMED GLOBAL from its first
 * arm rather than a local `.L` table, which is why the earlier family sweep
 * missed it -- that sweep matched only on `.L` returns.
 *
 * See src/overlays/rom_79aad8/ovl_314_a.c for why the address arithmetic is
 * spelled out and why the compared constant has to be a symbol.
 */
typedef struct { unsigned char _bytes[704]; } GlobalState;
extern GlobalState gState;
extern int _AREA_3c;
extern unsigned char gScript_943__0200c7a8[];
extern unsigned char L4838[] __asm__(".L4838");

unsigned char *OvlFunc_926_200834c(void)
{
    unsigned int base;
    unsigned int off;

    base = (unsigned int)&gState;
    off = 0xe0;
    off <<= 1;
    base += off;
    off = 0;
    if (*(short *)((char *)base + off) == (int)(&_AREA_3c))
        return gScript_943__0200c7a8;
    return L4838;
}
