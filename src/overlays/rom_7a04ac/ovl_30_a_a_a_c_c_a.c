/* OvlFunc_913_200834c -- FindPushableFacingPlayer, overlay rom_7a04ac, ovl_30_a_a_a_c_c_a.
 *
 ...
 *
 *   OK OvlFunc_913_200834c -- 296 bytes, 143 encodings and 5 relocations identical
 ...
 *   overlays/rom_7a04ac/overlay.ld:22
 *       asm/overlays/rom_7a04ac/ovl_30_a_a_a_c_c_a.o(.text)
 * The `.L` table symbols reached through __asm__ labels below are defined in
 * asm/overlays/rom_7a04ac/ovl_30_c_c_c_c_c.s and are ALREADY `.global`
...
extern int gStep_913__0200ad68[] __asm__(".L2d68");
extern int gModelId_913__0200ada8[] __asm__(".L2da8");
extern struct Rect gBox_913__0200adc0[] __asm__(".L2dc0");
extern struct Ent *__MapActor_GetActor(int slot);

struct Ent *OvlFunc_913_200834c(int *facingOut, int *slotOut, int *modelOut)
{
    struct Ent **tbl;
    struct Ent *pl;
    struct Ent *e;
    ...
    *facingOut = pl->facing >> 12;
    for (slot = 8; slot <= 0x41; slot++) {
        e = tbl[slot];
        for (i = 0; i <= 5; i++) {
            if (*e->f50->f28 != gModelId_913__0200ada8[i])
                continue;
            *modelOut = i;
            tx = ((pl->x >> 16) + (gStep_913__0200ad68[*facingOut] >> 16)) >> 4;
            tz = ((pl->z >> 16) + (short)gStep_913__0200ad68[*facingOut]) >> 4;
            ex = *(short *)((char *)e + 0xa);
            x0 = (ex + gBox_913__0200adc0[i].x0) >> 4;
            ez = *(short *)((char *)e + 0x12);
            z0 = (ez + gBox_913__0200adc0[i].z0) >> 4;
            x1 = (ex + gBox_913__0200adc0[i].x1) >> 4;
            z1 = (ez + gBox_913__0200adc0[i].z1) >> 4;
            if (x0 > tx)
                continue;
            if (tx >= x1)
                continue;
