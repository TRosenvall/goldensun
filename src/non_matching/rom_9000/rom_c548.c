 ...
 *
 *   2. the ROM keeps the actor pointer in r0, reusing the entity register;
 ...
 */
void Func_800c548(Entity *entity, s32 priority)
{
    if (entity != NULL && entity->drawKind == 1) {
        struct DrawActor *actor = (struct DrawActor *)entity->actor;

        actor->oamFlags = (actor->oamFlags & ~0xc) | ((priority & 3) << 2);
    }
}
 ...
 */
void Func_800c570(Entity *entity, s32 useCallerScale)
{
    if (entity != NULL && entity->drawKind == 1) {
        struct DrawActor *actor = (struct DrawActor *)entity->actor;

        actor->scaleFlags = (actor->scaleFlags & ~2) | ((useCallerScale & 1) << 1);
    }
}
