...
This file proposes **963** of them (29.2%).
...
## Overlay 959 scripts — 87 functions
...
| `OvlFunc_959_200981c` | `0x0200981c` | `Ovl959_Step200981C` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009880` | `0x02009880` | `Ovl959_Step2009880` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
...
| `OvlFunc_959_2009918` | `0x02009918` | `Ovl959_Step2009918` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009980` | `0x02009980` | `Ovl959_Step2009980` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20099e8` | `0x020099e8` | `Ovl959_Step20099E8` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009a44` | `0x02009a44` | `Ovl959_Step2009A44` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009ab0` | `0x02009ab0` | `Ovl959_Step2009AB0` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009b24` | `0x02009b24` | `Ovl959_Step2009B24` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009be4` | `0x02009be4` | `Ovl959_Step2009BE4` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009c4c` | `0x02009c4c` | `Ovl959_Step2009C4C` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009ca4` | `0x02009ca4` | `Ovl959_Step2009CA4` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009cf0` | `0x02009cf0` | `Ovl959_Step2009CF0` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009d60` | `0x02009d60` | `Ovl959_Step2009D60` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009dc4` | `0x02009dc4` | `Ovl959_Step2009DC4` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009e2c` | `0x02009e2c` | `Ovl959_Step2009E2C` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a0cc` | `0x0200a0cc` | `Ovl959_Step200A0CC` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a134` | `0x0200a134` | `Ovl959_Step200A134` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a26c` | `0x0200a26c` | `Ovl959_Step200A26C` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a2a0` | `0x0200a2a0` | `Ovl959_Step200A2A0` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a2d4` | `0x0200a2d4` | `Ovl959_Step200A2D4` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a308` | `0x0200a308` | `Ovl959_Step200A308` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a38c` | `0x0200a38c` | `Ovl959_Step200A38C` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a410` | `0x0200a410` | `Ovl959_Step200A410` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a468` | `0x0200a468` | `Ovl959_Step200A468` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a4c0` | `0x0200a4c0` | `Ovl959_Step200A4C0` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a4e4` | `0x0200a4e4` | `Ovl959_Step200A4E4` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a504` | `0x0200a504` | `Ovl959_Step200A504` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a528` | `0x0200a528` | `Ovl959_Step200A528` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a52c` | `0x0200a52c` | `Ovl959_Step200A52C` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200c638` | `0x0200c638` | `Ovl959_Step200C638` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200c928` | `0x0200c928` | `Ovl959_Step200C928` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200c964` | `0x0200c964` | `Ovl959_Step200C964` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cb68` | `0x0200cb68` | `Ovl959_Step200CB68` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cb84` | `0x0200cb84` | `Ovl959_Step200CB84` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cba4` | `0x0200cba4` | `Ovl959_Step200CBA4` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cbc4` | `0x0200cbc4` | `Ovl959_Step200CBC4` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cbe4` | `0x0200cbe4` | `Ovl959_Step200CBE4` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cbfc` | `0x0200cbfc` | `Ovl959_Step200CBFC` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cd0c` | `0x0200cd0c` | `Ovl959_Step200CD0C` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cd4c` | `0x0200cd4c` | `Ovl959_Step200CD4C` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cd50` | `0x0200cd50` | `Ovl959_Step200CD50` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cd88` | `0x0200cd88` | `Ovl959_Step200CD88` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200d470` | `0x0200d470` | `Ovl959_Step200D470` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200d4b0` | `0x0200d4b0` | `Ovl959_Step200D4B0` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200d4dc` | `0x0200d4dc` | `Ovl959_Step200D4DC` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200d520` | `0x0200d520` | `Ovl959_Step200D520` | read+family | Overlay 959 scripts | One of a run of near-identical actor-position steps in this map's script set: read the actor's x/z, compare against a stored target and advance. They differ only in slots and thresholds, so the address is the honest distinguisher. | `src/overlays/rom_7e7574/script.c` |
