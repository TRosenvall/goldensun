...
This file proposes **959** of them (29.1%).
...
| Function | Address | Proposed | Basis | ROM area | Why the name | Suggested home |
|---|---|---|---|---|---|---|
| `OvlFunc_883_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the _c part stays as assembly and keeps its slot in FindEntityAtPosition. Scans entity slots 8..0x41 -- the map-object range, above the party slots -- and returns the first whose position matches, o | `src/overlays/common/map_query.c` |
| `OvlFunc_883_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the _a part stays as assembly and keeps its slot in FillMapRectCollisionByte. Writes one byte into every cell of a rectangle of the map's cell array. Rows are 0x200 bytes apart, cells are 4 bytes, a | `src/overlays/common/map_query.c` |
| `OvlFunc_905_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_905_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_913_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_913_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_914_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_914_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_915_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_915_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_923_2008350` | `0x02008350` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_923_2008528` | `0x02008528` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_924_2008350` | `0x02008350` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_924_2008528` | `0x02008528` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_927_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_927_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_934_2008350` | `0x02008350` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_934_2008528` | `0x02008528` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_946_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_946_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_947_2008350` | `0x02008350` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of sixteen identical copies -- one per overlay, byte-for-byte the same body. See src | `src/overlays/common/map_query.c` |
| `OvlFunc_947_2008528` | `0x02008528` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling part stays as assembly and keeps its slot in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. See  | `src/overlays/common/map_query.c` |
| `OvlFunc_948_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of seventeen identical copies -- one per overlay, byte-for-byte the same body. See s | `src/overlays/common/map_query.c` |
| `OvlFunc_948_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. Se | `src/overlays/common/map_query.c` |
| `OvlFunc_957_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of seventeen identical copies -- one per overlay, byte-for-byte the same body. See s | `src/overlays/common/map_query.c` |
| `OvlFunc_957_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. Se | `src/overlays/common/map_query.c` |
| `OvlFunc_958_2008350` | `0x02008350` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of seventeen identical copies -- one per overlay, byte-for-byte the same body. See s | `src/overlays/common/map_query.c` |
| `OvlFunc_958_2008528` | `0x02008528` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. Se | `src/overlays/common/map_query.c` |
| `OvlFunc_959_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of seventeen identical copies -- one per overlay, byte-for-byte the same body. See s | `src/overlays/common/map_query.c` |
| `OvlFunc_959_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. Se | `src/overlays/common/map_query.c` |
| `OvlFunc_964_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of seventeen identical copies -- one per overlay, byte-for-byte the same body. See s | `src/overlays/common/map_query.c` |
| `OvlFunc_964_2008244` | `0x02008244` | `FillMapRectCollisionByte` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FillMapRectCollisionByte, one of eighteen identical copies -- one per overlay, byte-for-byte the same body. Se | `src/overlays/common/map_query.c` |
| `OvlFunc_965_200806c` | `0x0200806c` | `FindEntityAtPosition` | named | Overlay / map query | Split out of that .s; the sibling parts stay as assembly and keep their slots in the overlay's linker script. FindEntityAtPosition, one of seventeen identical copies -- one per overlay, byte-for-byte the same body. See s | `src/overlays/common/map_query.c` |
...
| Function | Address | Proposed | Basis | ROM area | Why the name | Suggested home |
|---|---|---|---|---|---|---|
| `OvlFunc_946_2008030` | `0x02008030` | `Ovl946_Distance2D` | read | Overlay 946 scripts | Returns the planar distance between two positions through the ARM absolute-value helper. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20082a8` | `0x020082a8` | `Ovl946_TestPropCollision` | read+callee | Overlay 946 scripts | Looks a prop up in the .L315c/.L319c tables and runs the engine collision test against it. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008474` | `0x02008474` | `Ovl946_FindPropAt` | read+callee | Overlay 946 scripts | Resolves a position to one of the props listed in .L31b4 and .L315c. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008608` | `0x02008608` | `Ovl946_ClearPropCollision` | read+callee | Overlay 946 scripts | Clears a prop's collision footprint from the map through FillMapRectCollisionByte. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20089dc` | `0x020089dc` | `Ovl946_SetSpriteSelect` | read+family | Overlay 946 scripts | Writes the two-bit selector inside the sprite byte at +9 as a bitfield -- the batch-71 narrow-constant shape. One of two copies here. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20089f4` | `0x020089f4` | `Ovl946_SpawnProp` | read+callee | Overlay 946 scripts | Creates a prop actor, sets its sprite flags, seeds its idle timer and its selector. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008a4c` | `0x02008a4c` | `Ovl946_SetSpriteSelectB` | read+family | Overlay 946 scripts | The second copy of Ovl946_SetSpriteSelect. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008ab0` | `0x02008ab0` | `Ovl946_SetDrawAngle` | read+family | Overlay 946 scripts | Writes the draw record's angle halfword at +0x1e. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008cc0` | `0x02008cc0` | `NullSub_Ovl946_2008cc0` | read | Overlay 946 scripts | Empty body. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008cc4` | `0x02008cc4` | `Ovl946_SelectAreaName` | read+family | Overlay 946 scripts | Picks one of the _AREA_* name ids from the global state. One of three near-identical selectors in this overlay. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008d3c` | `0x02008d3c` | `Ovl946_ReturnZero` | read | Overlay 946 scripts | Returns 0 unconditionally. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008d40` | `0x02008d40` | `Ovl946_GetScriptBase` | read | Overlay 946 scripts | Returns the overlay's own data block at gOvl_0200b508. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008d48` | `0x02008d48` | `Ovl946_SelectAreaNameB` | read+family | Overlay 946 scripts | The second area-name selector. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008da4` | `0x02008da4` | `Ovl946_UpdateDrawAngle` | read+family | Overlay 946 scripts | Recomputes a draw record's angle from its actor. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008e88` | `0x02008e88` | `Ovl946_CheckAreaFlag` | read | Overlay 946 scripts | Tests one of the map's progress flags in the global state. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008ec4` | `0x02008ec4` | `Ovl946_SelectAreaNameC` | read+family | Overlay 946 scripts | The third area-name selector. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2008f3c` | `0x02008f3c` | `Ovl946_StepEffect` | read | Overlay 946 scripts | Advances one of the map's ambient effects a frame. Which effect is not established by the body. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009214` | `0x02009214` | `Ovl946_StepProp` | read | Overlay 946 scripts | Advances a prop's position record by one frame. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20092b4` | `0x020092b4` | `Ovl946_RunAreaEvent` | read+callee | Overlay 946 scripts | Runs the area's flag-gated event: reads the progress flag, moves the actor and sets the completion flag. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009374` | `0x02009374` | `Ovl946_RunIntroCutscene` | read+callee | Overlay 946 scripts | Opens a cutscene, runs the map's setup and its area event, then closes. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200938c` | `0x0200938c` | `Ovl946_PlayDoorSound` | read+callee | Overlay 946 scripts | Runs the door tile edit, plays sound 0xb7 and bumps the map event counter. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20093ac` | `0x020093ac` | `Ovl946_RunDoorEvent` | read+callee | Overlay 946 scripts | The flag-gated door sequence for this map. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009508` | `0x02009508` | `Ovl946_OpenGate1` | read+family | Overlay 946 scripts | Reveals a gate actor, clears its collision and sets its flag. First of three identical gates differing only in slot and flag id. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009548` | `0x02009548` | `Ovl946_OpenGate2` | read+family | Overlay 946 scripts | The second gate. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200958c` | `0x0200958c` | `Ovl946_OpenGate3` | read+family | Overlay 946 scripts | The third gate. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20095d0` | `0x020095d0` | `Ovl946_ClearActorFlag59` | read | Overlay 946 scripts | Clears the byte at +0x59 of an actor record -- the collidable flag. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009624` | `0x02009624` | `Ovl946_RevealArea` | read+callee | Overlay 946 scripts | Sets the reveal flag and copies the revealed tiles into the map buffer. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200964c` | `0x0200964c` | `Ovl946_SetFlag8C5` | read+family | Overlay 946 scripts | Sets flag 0x8c5. One of three one-line flag setters. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200965c` | `0x0200965c` | `Ovl946_SetFlag8C6` | read+family | Overlay 946 scripts | Sets flag 0x8c6. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200966c` | `0x0200966c` | `Ovl946_SetFlag8C7` | read+family | Overlay 946 scripts | Sets flag 0x8c7. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200967c` | `0x0200967c` | `Ovl946_PlacePropsForFlags` | read+callee | Overlay 946 scripts | Positions the map's props according to which progress flags are set. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009740` | `0x02009740` | `Ovl946_RefreshPropCollision` | read+callee | Overlay 946 scripts | Re-runs the find-and-clear pass over every prop's collision footprint. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200985c` | `0x0200985c` | `Ovl946_MoveActorA` | read+family | Overlay 946 scripts | Moves one of the map's actors along a stored path. First of a pair. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20098b0` | `0x020098b0` | `Ovl946_MoveActorB` | read+family | Overlay 946 scripts | The second of the pair. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009904` | `0x02009904` | `Ovl946_RunScene` | read | Overlay 946 scripts | The scene dispatcher: fourteen one-line wrappers call it with a scene number. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200991c` | `0x0200991c` | `Ovl946_MoveActorsBoth` | read+callee | Overlay 946 scripts | Runs both actor movers in sequence. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20099b4` | `0x020099b4` | `Ovl946_RunScene08` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 8. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20099c0` | `0x020099c0` | `Ovl946_RunScene10` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 10. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20099cc` | `0x020099cc` | `Ovl946_RunScene11` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 11. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20099d8` | `0x020099d8` | `Ovl946_RunScene12` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 12. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20099e4` | `0x020099e4` | `Ovl946_RunScene13` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 13. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20099f0` | `0x020099f0` | `Ovl946_RunScene15` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 15. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_20099fc` | `0x020099fc` | `Ovl946_RunScene17` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 17. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009a08` | `0x02009a08` | `Ovl946_RunScene18` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 18. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009a14` | `0x02009a14` | `Ovl946_RunScene09` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 9. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009a20` | `0x02009a20` | `Ovl946_RunScene19` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 19. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009a2c` | `0x02009a2c` | `Ovl946_RunScene14` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 14. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009a38` | `0x02009a38` | `Ovl946_RunScene16` | read+family | Overlay 946 scripts | A one-line wrapper calling Ovl946_RunScene with scene number 16. One of twelve identical wrappers differing only in that constant. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009a44` | `0x02009a44` | `Ovl946_RunActorCutscene` | read+callee | Overlay 946 scripts | Opens a cutscene, animates an actor and closes it again. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009b14` | `0x02009b14` | `Ovl946_GetActorPos` | read+family | Overlay 946 scripts | Reads an actor's three position words. First of two copies. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009b40` | `0x02009b40` | `Ovl946_RunCutsceneAtPos` | read+family | Overlay 946 scripts | Runs the actor cutscene at a position read into a stack buffer. First of two. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009b68` | `0x02009b68` | `Ovl946_GetActorPosB` | read+family | Overlay 946 scripts | The second position reader. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009b94` | `0x02009b94` | `Ovl946_RunCutsceneAtPosB` | read+family | Overlay 946 scripts | The second positioned cutscene. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009bbc` | `0x02009bbc` | `Ovl946_MapEditSeq2009BBC` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009de0` | `0x02009de0` | `Ovl946_MapEditSeq2009DE0` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009e5c` | `0x02009e5c` | `Ovl946_MapEditSeq2009E5C` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009ef4` | `0x02009ef4` | `Ovl946_MapEditSeq2009EF4` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_2009f78` | `0x02009f78` | `Ovl946_MapEditSeq2009F78` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a004` | `0x0200a004` | `Ovl946_MapEditSeq200A004` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a080` | `0x0200a080` | `Ovl946_MapEditSeq200A080` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a200` | `0x0200a200` | `Ovl946_MapEditSeq200A200` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a2c8` | `0x0200a2c8` | `Ovl946_MapEditSeq200A2C8` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a450` | `0x0200a450` | `Ovl946_MapEditSeq200A450` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a4c8` | `0x0200a4c8` | `Ovl946_MapEditSeq200A4C8` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a5f0` | `0x0200a5f0` | `Ovl946_MapEditSeq200A5F0` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a700` | `0x0200a700` | `Ovl946_MapEditSeq200A700` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a848` | `0x0200a848` | `Ovl946_MapEditSeq200A848` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200a984` | `0x0200a984` | `Ovl946_MapEditSeq200A984` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200aa98` | `0x0200aa98` | `Ovl946_MapEditSeq200AA98` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200ab80` | `0x0200ab80` | `Ovl946_MapEditSeq200AB80` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200ac4c` | `0x0200ac4c` | `Ovl946_MapEditSeq200AC4C` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200ad0c` | `0x0200ad0c` | `Ovl946_MapEditSeq200AD0C` | read+family | Overlay 946 scripts | One of nineteen near-identical cutscene steps in this map: fetch an actor, wait frames, copy a tile rect with Map_CopyRect and call the shared step helper. They differ only in slots, frame counts and rects, so the address suffix is the honest distinguisher. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200ae70` | `0x0200ae70` | `Ovl946_StepActorPath` | read | Overlay 946 scripts | Advances an actor one step along its path record. | `src/overlays/rom_7ced6c/script.c` |
| `OvlFunc_946_200aed8` | `0x0200aed8` | `Ovl946_SetSpriteBits` | read+family | Overlay 946 scripts | Writes the five-bit and one-bit fields inside the sprite byte at +5. | `src/overlays/rom_7ced6c/script.c` |

## Overlay 959 scripts — 83 functions

| Function | Address | Proposed | Basis | ROM area | Why the name | Suggested home |
|---|---|---|---|---|---|---|
| `OvlFunc_959_2008030` | `0x02008030` | `Ovl959_Distance2D` | read+family | Overlay 959 scripts | Planar distance between two positions through the ARM absolute-value helper. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20082a8` | `0x020082a8` | `Ovl959_TestPropCollision` | read+callee | Overlay 959 scripts | Looks a prop up in .L5ed8/.L5f18 and runs the engine collision test against it. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008474` | `0x02008474` | `Ovl959_FindPropAt` | read+callee | Overlay 959 scripts | Resolves a position to one of the props listed in .L5f30 and .L5ed8. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008608` | `0x02008608` | `Ovl959_ClearPropCollision` | read+callee | Overlay 959 scripts | Clears a prop's collision footprint from the map. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20089dc` | `0x020089dc` | `Ovl959_SelectAreaName` | read+family | Overlay 959 scripts | Picks one of the _AREA_* name ids from the global state. One of four near-identical selectors in this overlay. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008a30` | `0x02008a30` | `Ovl959_ReturnZero` | read | Overlay 959 scripts | Returns 0 unconditionally. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008a34` | `0x02008a34` | `Ovl959_SelectAreaNameB` | read+family | Overlay 959 scripts | The second area-name selector. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008a80` | `0x02008a80` | `Ovl959_SelectAreaNameC` | read+family | Overlay 959 scripts | The third area-name selector. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008af8` | `0x02008af8` | `Ovl959_SelectAreaNameD` | read+family | Overlay 959 scripts | The fourth area-name selector. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008b4c` | `0x02008b4c` | `Ovl959_RevealTiles` | read+callee | Overlay 959 scripts | Fetches an actor and copies the tiles its position reveals into the map buffer. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008ba0` | `0x02008ba0` | `Ovl959_RunMapSetup` | read+callee | Overlay 959 scripts | A pure forward to the overlay's map setup routine. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008bec` | `0x02008bec` | `Ovl959_RunCameraCutscene` | read+callee | Overlay 959 scripts | Moves the camera, edits the map tiles and plays a sound -- the map's scripted reveal. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008c90` | `0x02008c90` | `Ovl959_DrawPuzzleRowA` | read+family | Overlay 959 scripts | Draws one puzzle row from the table at .L7714. First of three row drawers differing only in their table. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008ce0` | `0x02008ce0` | `Ovl959_CheckPuzzleItemA` | read+family | Overlay 959 scripts | Flag-gated check that the party holds the required item for this puzzle stage. First of four. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008dcc` | `0x02008dcc` | `Ovl959_CheckPuzzleItemB` | read+family | Overlay 959 scripts | The second item check, with its own sound and flag. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008e30` | `0x02008e30` | `Ovl959_DrawPuzzleRowB` | read+family | Overlay 959 scripts | The .L7754 row drawer. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008e80` | `0x02008e80` | `Ovl959_CheckPuzzleItemC` | read+family | Overlay 959 scripts | The third item check. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008ee0` | `0x02008ee0` | `Ovl959_DrawPuzzleRowC` | read+family | Overlay 959 scripts | The .L7764 row drawer, which also copies a tile rect. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008f30` | `0x02008f30` | `Ovl959_CheckPuzzleItemD` | read+family | Overlay 959 scripts | The fourth item check. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008f94` | `0x02008f94` | `Ovl959_GetPuzzleState` | read | Overlay 959 scripts | Returns the puzzle's current state word. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008fb4` | `0x02008fb4` | `Ovl959_ShowMessage953` | read | Overlay 959 scripts | Opens message 0x953. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008fc8` | `0x02008fc8` | `Ovl959_TrySolveStage8` | read+family | Overlay 959 scripts | Runs the shared stage check with (8, 8) and sets its flag on success. One of four stage wrappers. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2008fe4` | `0x02008fe4` | `Ovl959_TrySolveStage9` | read+family | Overlay 959 scripts | The (9, 7) stage. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009000` | `0x02009000` | `Ovl959_TrySolveStageA` | read+family | Overlay 959 scripts | The (0xa, 6) stage, setting flag 0xf2c. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200901c` | `0x0200901c` | `Ovl959_TrySolveStageB` | read+family | Overlay 959 scripts | The (0xb, 5) stage. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009038` | `0x02009038` | `Ovl959_CheckStage` | read | Overlay 959 scripts | The shared stage check the four wrappers call: compares two slots and sets the actor animation. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009098` | `0x02009098` | `NullSub_Ovl959_2009098` | read+family | Overlay 959 scripts | Empty body; one of four consecutive empty subs in this file. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20090a8` | `0x020090a8` | `Ovl959_PlaceActorsForScene` | read+callee | Overlay 959 scripts | Sets the scene's actor animations and positions. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009108` | `0x02009108` | `Ovl959_GetPlayerPos` | read+family | Overlay 959 scripts | Reads slot 0's position words. One of four identical readers in this overlay. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20092e0` | `0x020092e0` | `Ovl959_WalkActorAndFace` | read+callee | Overlay 959 scripts | Moves an actor by an offset, sets its animation and turns it to face a target. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009324` | `0x02009324` | `Ovl959_GetActorPosPair` | read+family | Overlay 959 scripts | Reads two actors' positions into locals. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20094bc` | `0x020094bc` | `Ovl959_EndScene11` | read+family | Overlay 959 scripts | Runs scene 0x11 and closes the cutscene. One of three scene-and-close wrappers. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20094cc` | `0x020094cc` | `Ovl959_GetPlayerPosB` | read+family | Overlay 959 scripts | The second slot-0 position reader. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009650` | `0x02009650` | `Ovl959_EndScene12` | read+family | Overlay 959 scripts | The scene 0x12 wrapper. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009660` | `0x02009660` | `Ovl959_GetPlayerPosC` | read+family | Overlay 959 scripts | The third slot-0 position reader. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200969c` | `0x0200969c` | `Ovl959_CheckAreaProgress` | read | Overlay 959 scripts | Compares the map block against the global state's progress fields. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009708` | `0x02009708` | `Ovl959_EndScene11B` | read+family | Overlay 959 scripts | The second scene 0x11 wrapper. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009718` | `0x02009718` | `Ovl959_RunStageBranchA` | read+family | Overlay 959 scripts | Dispatches on the map's stage counter through the shared stage resolver. First of three. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200975c` | `0x0200975c` | `Ovl959_RunStageBranchB` | read+family | Overlay 959 scripts | The second stage branch. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20097bc` | `0x020097bc` | `Ovl959_RunStageBranchC` | read+family | Overlay 959 scripts | The third stage branch. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200981c` | `0x0200981c` | `Ovl959_IsPlayerNearA` | read+family | Overlay 959 scripts | Compares slot 0's x and z against a stored point and reports proximity. First of five near-identical proximity tests differing in their thresholds. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009880` | `0x02009880` | `Ovl959_IsPlayerNearB` | read+family | Overlay 959 scripts | The second proximity test. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20098e4` | `0x020098e4` | `Ovl959_ResolveStage` | read+callee | Overlay 959 scripts | The shared stage resolver the three branches call, choosing between the two position tests. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009918` | `0x02009918` | `Ovl959_IsPlayerNearC` | read+family | Overlay 959 scripts | The third proximity test. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009980` | `0x02009980` | `Ovl959_IsPlayerNearD` | read+family | Overlay 959 scripts | The fourth proximity test. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_20099e8` | `0x020099e8` | `Ovl959_IsPlayerNearE` | read+family | Overlay 959 scripts | The fifth proximity test. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009a44` | `0x02009a44` | `Ovl959_IsPlayerFacingProp` | read+family | Overlay 959 scripts | The proximity test that also reads the actor's facing halfword at +6. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009ab0` | `0x02009ab0` | `Ovl959_RunGuardCutsceneA` | read+family | Overlay 959 scripts | Opens a cutscene, walks an actor, sets its behaviour and idle, delivers _MSG_240d and closes. First of three variants. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009b24` | `0x02009b24` | `Ovl959_RunGuardCutsceneB` | read+family | Overlay 959 scripts | The variant that adds an emote before the line. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009be4` | `0x02009be4` | `Ovl959_RunDialogueByIndex` | read+callee | Overlay 959 scripts | Dispatches to one of four dialogue routines using the index table at .L5fa4. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009c4c` | `0x02009c4c` | `Ovl959_SayLine241E` | read+family | Overlay 959 scripts | Delivers message 0x241e with its animation. One of four single-line dialogue routines named by their message id. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009ca4` | `0x02009ca4` | `Ovl959_SayLine2421` | read+family | Overlay 959 scripts | Delivers message 0x2421. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009cf0` | `0x02009cf0` | `Ovl959_SayLine2424` | read+family | Overlay 959 scripts | Delivers message 0x2424. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009d60` | `0x02009d60` | `Ovl959_SayLine2428` | read+family | Overlay 959 scripts | Delivers message 0x2428. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009dc4` | `0x02009dc4` | `Ovl959_RunDialogueForProgressA` | read+family | Overlay 959 scripts | Picks a dialogue index from the map block and the global progress state, then runs it. First of three. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_2009e2c` | `0x02009e2c` | `Ovl959_RunDialogueForProgressB` | read+family | Overlay 959 scripts | The second progress-driven dialogue selector. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a0cc` | `0x0200a0cc` | `Ovl959_RunDialogueForProgressC` | read+family | Overlay 959 scripts | The third progress-driven dialogue selector. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a134` | `0x0200a134` | `Ovl959_RunGuardCutsceneC` | read+family | Overlay 959 scripts | The third guard cutscene variant. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a26c` | `0x0200a26c` | `Ovl959_EditMapA` | read+family | Overlay 959 scripts | A pair of tile edits applied back to back. First of five such map edits in this overlay, differing only in their rects. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a2a0` | `0x0200a2a0` | `Ovl959_EditMapB` | read+family | Overlay 959 scripts | The second map edit. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a2d4` | `0x0200a2d4` | `Ovl959_EditMapC` | read+family | Overlay 959 scripts | The third map edit. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a308` | `0x0200a308` | `Ovl959_RunChestEventA` | read+family | Overlay 959 scripts | Flag-gated pickup: checks the flag, sets it, opens the message and plays the sound. First of two. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a38c` | `0x0200a38c` | `Ovl959_RunChestEventB` | read+family | Overlay 959 scripts | The second pickup event. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a410` | `0x0200a410` | `Ovl959_EditMapD` | read+family | Overlay 959 scripts | The fourth map edit. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a468` | `0x0200a468` | `Ovl959_EditMapE` | read+family | Overlay 959 scripts | The fifth map edit. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a4c0` | `0x0200a4c0` | `Ovl959_ResetSceneState` | read | Overlay 959 scripts | Zeroes the scene's two working values before a sequence runs. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a4e4` | `0x0200a4e4` | `Ovl959_SetFlag945AndEdit` | read+callee | Overlay 959 scripts | Sets flag 0x945 and applies the third map edit. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a504` | `0x0200a504` | `Ovl959_SetFlag946AndMove` | read+callee | Overlay 959 scripts | Sets flag 0x946, clears the area flag and parks actor 0xa at the origin. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a528` | `0x0200a528` | `NullSub_Ovl959_200a528` | read | Overlay 959 scripts | Empty body. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200a52c` | `0x0200a52c` | `Ovl959_StageActorsForScene` | read+callee | Overlay 959 scripts | Places the scene's actors, gives them their wander scripts and speeds, and starts them moving. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200c638` | `0x0200c638` | `Ovl959_RunActorDialogue` | read+callee | Overlay 959 scripts | Runs a full actor dialogue from the .L5fa4 index table, waiting between lines. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200c928` | `0x0200c928` | `Ovl959_SayIfFlag941A` | read+family | Overlay 959 scripts | Delivers one of two lines depending on flag 0x941. First of a pair sharing a file. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200c964` | `0x0200c964` | `Ovl959_SayIfFlag941B` | read+family | Overlay 959 scripts | The second flag-gated line. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cb68` | `0x0200cb68` | `Ovl959_RunEffectAndSetFlag94E` | read | Overlay 959 scripts | Runs the engine effect with (0x1a, 1, 5) and sets flag 0x94e. Which effect that is is not established by the body. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cb84` | `0x0200cb84` | `Ovl959_SayLine2441` | read+family | Overlay 959 scripts | Actor 0xe delivers message 0x2441. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cba4` | `0x0200cba4` | `Ovl959_SayLine2440` | read+family | Overlay 959 scripts | Actor 0xd delivers message 0x2440. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cbc4` | `0x0200cbc4` | `Ovl959_SayLineActor0C` | read+family | Overlay 959 scripts | Actor 0xc's dialogue line. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cbe4` | `0x0200cbe4` | `Ovl959_SayLine2459` | read+family | Overlay 959 scripts | Message 0x2459 delivered through the shared actor-message helper. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cbfc` | `0x0200cbfc` | `Ovl959_SayProgressLine` | read | Overlay 959 scripts | Chooses between _MSG_242e and _MSG_2430 on a progress flag and sets the seen flag. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cd0c` | `0x0200cd0c` | `Ovl959_RunMinimapDialogue` | read+callee | Overlay 959 scripts | Flag-gated dialogue that also stops the minimap task. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cd4c` | `0x0200cd4c` | `NullSub_Ovl959_200cd4c` | read | Overlay 959 scripts | Empty body. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cd50` | `0x0200cd50` | `Ovl959_SayItemLine256C` | read+callee | Overlay 959 scripts | Delivers _MSG_256c and branches on whether the party holds the required item. | `src/overlays/rom_7e7574/script.c` |
| `OvlFunc_959_200cd88` | `0x0200cd88` | `Ovl959_SayLine256D` | read+family | Overlay 959 scripts | Actor 0xd delivers message 0x256d. | `src/overlays/rom_7e7574/script.c` |
...
## Overlay scripts — 45 functions

| Function | Address | Proposed | Basis | ROM area | Why the name | Suggested home |
|---|---|---|---|---|---|---|
| `OvlFunc_879_20081c0` | `0x020081c0` | `LoadTitleSprite` | named | Overlay scripts | OvlFunc_879_20081c0 -- 0x020081c0 Cut out of goldensun/asm/overlays/rom_779188/ovl_30_c_c_b.s. Loads the cursor sprite: allocate a scratch buffer, claim a sprite slot the first time only, decompress, DMA the palette, upl | `src/overlays/rom_779188/script.c` |
| `OvlFunc_880_20092c8` | `0x020092c8` | `Crc16Ccitt` | named | Overlay scripts | unsigned int OvlFunc_880_20092c8(unsigned int n, unsigned char *p) { unsigned int crc; unsigned int i; int j; crc = 0xffff; i = 0; if (n != 0) { do { crc ^= p[0] << 8; j = 0; do { if ((crc & 0x8000) != 0) crc = (crc << 1 | `src/overlays/rom_7795e8/script.c` |
| `OvlFunc_895_2008200` | `0x02008200` | `TalkPassageA` | named | Overlay scripts | TalkPassageA. Says line 0x1034 once the passage has been opened (save bit 0x81a), otherwise 0x1031 -- and in the not-yet-opened case, if the arming bit 0xf01 is set, writes 1 to [iwram_3001ebc]+0x172. A SECOND MEMBER of  | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_200856c` | `0x0200856c` | `TrackBlock9West` | named | Overlay scripts | Slotted between ovl_30_c_c_a_c_a.o and the rest of the overlay. EIGHT NEAR-TWINS IN ONE OBJECT, AND THAT IS DELIBERATE. All eight need -fno-rerun-cse-after-loop and they are contiguous in the .s, so splitting once puts t | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_20085ac` | `0x020085ac` | `TrackBlockAWest` | named | Overlay scripts | Slotted between ovl_30_c_c_a_c_a.o and the rest of the overlay. EIGHT NEAR-TWINS IN ONE OBJECT, AND THAT IS DELIBERATE. All eight need -fno-rerun-cse-after-loop and they are contiguous in the .s, so splitting once puts t | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_20085ec` | `0x020085ec` | `TrackBlock9East` | named | Overlay scripts | Slotted between ovl_30_c_c_a_c_a.o and the rest of the overlay. EIGHT NEAR-TWINS IN ONE OBJECT, AND THAT IS DELIBERATE. All eight need -fno-rerun-cse-after-loop and they are contiguous in the .s, so splitting once puts t | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_2008634` | `0x02008634` | `TrackBlockAEast` | named | Overlay scripts | Slotted between ovl_30_c_c_a_c_a.o and the rest of the overlay. EIGHT NEAR-TWINS IN ONE OBJECT, AND THAT IS DELIBERATE. All eight need -fno-rerun-cse-after-loop and they are contiguous in the .s, so splitting once puts t | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_200867c` | `0x0200867c` | `TrackBlockBEast` | named | Overlay scripts | Slotted between ovl_30_c_c_a_c_a.o and the rest of the overlay. EIGHT NEAR-TWINS IN ONE OBJECT, AND THAT IS DELIBERATE. All eight need -fno-rerun-cse-after-loop and they are contiguous in the .s, so splitting once puts t | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_20086c4` | `0x020086c4` | `TrackBlockCEast` | named | Overlay scripts | Slotted between ovl_30_c_c_a_c_a.o and the rest of the overlay. EIGHT NEAR-TWINS IN ONE OBJECT, AND THAT IS DELIBERATE. All eight need -fno-rerun-cse-after-loop and they are contiguous in the .s, so splitting once puts t | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_200870c` | `0x0200870c` | `TrackBlockDEast` | named | Overlay scripts | Slotted between ovl_30_c_c_a_c_a.o and the rest of the overlay. EIGHT NEAR-TWINS IN ONE OBJECT, AND THAT IS DELIBERATE. All eight need -fno-rerun-cse-after-loop and they are contiguous in the .s, so splitting once puts t | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_2008754` | `0x02008754` | `TrackBlockEEast` | named | Overlay scripts | Slotted between ovl_30_c_c_a_c_a.o and the rest of the overlay. EIGHT NEAR-TWINS IN ONE OBJECT, AND THAT IS DELIBERATE. All eight need -fno-rerun-cse-after-loop and they are contiguous in the .s, so splitting once puts t | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_200892c` | `0x0200892c` | `SetupArea13` | named | Overlay scripts | The target was the FIRST of six functions, so there is no _a part. Area-entry fixups behind three independent flag guards. TWO SPELLINGS ARE LOAD-BEARING: 1. z (the stored zero) is assigned BEFORE q (the destination addr | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_895_2009ac8` | `0x02009ac8` | `ShakeTask` | named | Overlay scripts | extern int L269c __asm__(".L269c"); extern int __Random(void); extern void __PlaySound(int id); extern void __Func_8012330(int a, int b, int c); void OvlFunc_895_2009ac8(void) { int *p; int v; int m1, m2; int e; int a1,  | `src/overlays/rom_78dee8/script.c` |
| `OvlFunc_900_2008094` | `0x02008094` | `GreetSlot8` | named | Overlay scripts | tools/asmfacts.py. GreetSlot8. The slot-8 NPC's scripted greeting: turn toward slots 9 and 0xA in turn forty frames apart, speak a line, re-form the followers, turn 8 and 0 to face each other, drop 8 back to formation 1, | `src/overlays/rom_797740/script.c` |
| `OvlFunc_902_2008204` | `0x02008204` | `TalkInventoryGate` | named | Overlay scripts | extern unsigned char *iwram_3001ebc; extern void __CutsceneStart(void); extern void __CutsceneEnd(void); extern void __CutsceneWait(int n); extern void __Func_8092848(int a, int b, int c); extern int __GetFlag(int id); e | `src/overlays/rom_7987ac/script.c` |
| `OvlFunc_902_2008570` | `0x02008570` | `SpawnTrackedObject` | named | Overlay scripts | Placed in the run in goldensun/overlays/rom_7987ac/overlay.ld. Byte-identical to OvlFunc_899_200c698 in overlays/rom_794ac0; this C is shared verbatim, with only the symbol changed. Spawns the item-icon actor: creates it | `src/overlays/rom_7987ac/script.c` |
| `OvlFunc_908_200835c` | `0x0200835c` | `TalkOnceThenRepeat` | named | Overlay scripts | extern int __GetFlag(int id); extern void __SetFlag(int id); extern void __CutsceneStart(void); extern void __CutsceneEnd(void); extern void __CutsceneWait(int n); extern void __MessageID(int id); extern void __ActorMess | `src/overlays/rom_79c0c4/script.c` |
| `OvlFunc_912_2008030` | `0x02008030` | `ResetRecordArray` | named | Overlay scripts | Overlay 912: initialise a fifteen-entry slot table. Whole-file conversion of asm/overlays/rom_7a0010/ovl_30_a_a.s. | `src/overlays/rom_7a0010/script.c` |
| `OvlFunc_916_2008b8c` | `0x02008b8c` | `FindRegionContaining` | named | Overlay scripts | OvlFunc_916_2008b8c -- asm/overlays/rom_7a37f0/ovl_30_c_c_c_a_c_a.s FindRegionContaining: walk 0x0c-byte records until the halfword at +0 is -1, return the first whose x/z bounds contain (x, z). The flag at +6 chooses wh | `src/overlays/rom_7a37f0/script.c` |
| `OvlFunc_919_200815c` | `0x0200815c` | `ApplyRasterBlend` | named | Overlay scripts | confirmed data-free by split_s.py. From the branch-over-pool class; the pool emitted at the ROM's position unaided, with the pool words in the ROM's order, reached with NO int intermediate. The recipe recorded elsewhere  | `src/overlays/rom_7a67d8/script.c` |
| `OvlFunc_919_200826c` | `0x0200826c` | `RasterSplitHandler` | named | Overlay scripts | Picks one of two scroll values by a VCOUNT threshold and writes it to BG3HOFS. A LEAF FUNCTION -- no calls at all, which is why it was reachable: no callee signature to guess and no argument setup to get wrong. Two spell | `src/overlays/rom_7a67d8/script.c` |
| `OvlFunc_919_20082a0` | `0x020082a0` | `UpdateRasterSplit` | named | Overlay scripts | OvlFunc_919_20082a0, the whole of goldensun/asm/overlays/rom_7a67d8/ovl_30_c_a_c_c.s. no linker-script change was needed. UpdateRasterSplit -- the per-frame task feeding OvlFunc_26c. Reads the camera record at [iwram_300 | `src/overlays/rom_7a67d8/script.c` |
| `OvlFunc_920_2008214` | `0x02008214` | `RevealSlotF` | named | Overlay scripts | OvlFunc_920_2008214 -- 0x02008214, cut from the tail of RevealSlotF: teleport slot 8 to the origin, set a save bit, and after a forty-frame beat bring slot 0xF into view -- animation 2, its flag byte at +0x55 cleared, bi | `src/overlays/rom_7a6ae4/script.c` |
| `OvlFunc_920_20084b4` | `0x020084b4` | `SetupArea31` | named | Overlay scripts | Slotted between ovl_30_c_c_a_a_a_a.o and the rest of the overlay. Stack-arg-pair lever, standard form: both values named, in the order the ROM stores them, immediately before the call. | `src/overlays/rom_7a6ae4/script.c` |
| `OvlFunc_924_200a844` | `0x0200a844` | `PaletteFadeToWhite` | named | Overlay scripts | extern void __WaitFrames(int n); void OvlFunc_924_200a844(void) { volatile unsigned short *p; unsigned int n; unsigned int i; int r, g, b; do { p = (volatile unsigned short *)0x5000050; n = 0; i = 0; do { r = *p & 0x1f;  | `src/overlays/rom_7ac2d8/script.c` |
| `OvlFunc_924_200ae08` | `0x0200ae08` | `PaletteFadeToWhite` | named | Overlay scripts | extern void __WaitFrames(int n); void OvlFunc_924_200ae08(void) { volatile unsigned short *p; unsigned int n; unsigned int i; int r, g, b; do { p = (volatile unsigned short *)0x50000c2; n = 0; i = 0; do { r = *p & 0x1f;  | `src/overlays/rom_7ac2d8/script.c` |
| `OvlFunc_929_2008524` | `0x02008524` | `TalkStaged` | named | Overlay scripts | TalkStaged. Slot 9 delivers a line, turns to face slot 0x0a for sixty frames, turns back to slot 0 for twenty, then delivers a closing line. THE PROTOTYPES BELOW ARE LOAD-BEARING -- see docs/elevation.md. An IMPLICITLY d | `src/overlays/rom_7b7790/script.c` |
| `OvlFunc_937_20081fc` | `0x020081fc` | `ShopCounter` | named | Overlay scripts | A sanctum attendant, in the SHIFTED-ADDITIVE variant of the facing test: ldr r2, =0x5fff / add r3, r2 / ldr r2, =0x3ffe0000 / lsl r3, #16 / cmp 0x5fff is -0xa001 taken modulo 0x10000, so on a u16 it is the same range as  | `src/overlays/rom_7c3044/script.c` |
| `OvlFunc_937_2008240` | `0x02008240` | `ClearSlotFlags` | named | Overlay scripts | OvlFunc_937_2008240 -- 0x02008240 ONE OF THE MAP-EXIT FAMILY. A search of every .s for the opening `push {r5,r6,r7,lr} / ldr r3, =iwram_3001ebc / ldr r7, [r3] bl __CutsceneStart / mov r5, #8 / mov r6, #0` returns six fun | `src/overlays/rom_7c3044/script.c` |
| `OvlFunc_937_200833c` | `0x0200833c` | `StageArea64` | named | Overlay scripts | Code to this file, the trailing .section .data to its _c sibling. OK on the first screen, on two reads. IT RETURNS void. The epilogue is `add sp, #8 / pop {r0} / bx r0` -- the `pop {r0}` tell -- with no `mov r0, #0`, so  | `src/overlays/rom_7c3044/script.c` |
| `OvlFunc_950_20087b0` | `0x020087b0` | `TalkTwoBits` | named | Overlay scripts | extern int _MSG_2399; extern int __GetFlag(int id); extern void __SetFlag(int id); extern void __CutsceneStart(void); extern void __CutsceneEnd(void); extern void __CutsceneWait(int n); extern void __MessageID(int id); e | `src/overlays/rom_7d5838/script.c` |
| `OvlFunc_964_2009038` | `0x02009038` | `WaitForField` | named | Overlay scripts | translation unit and the linker script is untouched. Waits up to 0x3c frames for the actor's field at +0xc to fall to or below the one at +0x14, then commits. THE LEVER IS THE LOOP SHAPE. The ROM's continue-test is the B | `src/overlays/rom_7ed0a0/script.c` |
| `OvlFunc_970_2008168` | `0x02008168` | `DecayCounter` | named | Overlay scripts | A LEAF FUNCTION, matched on the first screen. Ticks a counter and, every 0x28 ticks, decrements a second value down to a floor of 4 and resets the counter. Nothing clever is needed -- two early returns rather than nested | `src/overlays/rom_7fa4ec/script.c` |
| `OvlFunc_common0_0` | `` | `SetActorFacingBits` | named | Overlay scripts | First in the common0 run, ahead of the _a_b piece, in all NINETEEN overlay.ld scripts. Writes a two-bit selector into bits 2-3 of the sprite flag byte at +9 of the sprite record hanging off the actor at +0x50. FIVE BYTE- | `src/overlays/common/script.c` |
| `OvlFunc_common0_18` | `` | `SpawnPropForeground` | named | Overlay scripts | of the same .s in goldensun/overlays/rom_7b4558/overlay.ld. ONE OF FOUR OPERAND-IDENTICAL COPIES, elevated together from a single source with only the function name changed: OvlFunc_common0_18 ovl_7b4558 OvlFunc_964_2008 | `src/overlays/common/script.c` |
| `OvlFunc_common0_70` | `` | `SpawnPropBackground` | named | Overlay scripts | OvlFunc_common0_70 -- asm/overlays/common/common0_a_b.s Create an actor and initialise it; returns 0 if the spawn failed. TWO THINGS DECIDED IT. 1. THE NULL RETURN MUST BE INSIDE AN `else`-SHAPED TAIL. Written as the gua | `src/overlays/common/script.c` |
| `OvlFunc_common0_d4` | `` | `StepPropPhysics` | named | Overlay scripts | Slotted between the _a and _c pieces in all NINETEEN overlay.ld scripts that name this object. The _c piece keeps the .data and .data1 sections. Advances an entity by its per-axis deltas and spins its attached sprite. Th | `src/overlays/common/script.c` |
| `OvlFunc_common1_1254` | `` | `RunTurnAnimation` | named | Overlay scripts | extern unsigned char iwram_3001e68; extern unsigned char gState[]; extern int L49 __asm__(".L49"); extern int L20 __asm__(".L20"); extern int L31 __asm__(".L31"); struct Ent { unsigned char pad0[6]; unsigned char f6; uns | `src/overlays/common/script.c` |
| `OvlFunc_common1_1490` | `` | `StartSpriteEffect` | named | Overlay scripts | extern short L45[] __asm__(".L45"); extern short L29[] __asm__(".L29"); extern short L19[] __asm__(".L19"); extern short L26[] __asm__(".L26"); extern short L32[] __asm__(".L32"); extern void OvlFunc_common1_1334(void);  | `src/overlays/common/script.c` |
| `OvlFunc_common1_172c` | `` | `MaybeSpawnDustPuff` | named | Overlay scripts | OvlFunc_common1_172c -- shared by ovl_7db0c8, ovl_7ddb88 and ovl_7e0928 keeps its name (`asm/%.o: src/%.c`) and all three overlay linker scripts are unchanged. A one-in-ten chance, per call, of spawning a small effect ac | `src/overlays/common/script.c` |
| `OvlFunc_common1_17c0` | `` | `TeleportPlayerToProp` | named | Overlay scripts | OvlFunc_common1_17c0 -- common overlay, +0x17c0 Cut out of goldensun/asm/overlays/common/common1_c_a_c_c_a.s. Sends the actor whose slot id is stored in the caller's +0x64 field walking to the caller's own position, drop | `src/overlays/common/script.c` |
| `OvlFunc_common1_1fb4` | `` | `StartPuzzle` | named | Overlay scripts | Slotted between common1_c_a_c_c_a.o and the rest of the overlay. THE 1 STORED INTO THE FIRST TWO HALFWORDS IS A NAMED int, and both defects it causes come from the same omission. As the literal `1`, gcc-2.96 puts the con | `src/overlays/common/script.c` |
| `OvlFunc_common1_3e4` | `` | `RunPartySizeBranch` | named | Overlay scripts | extern unsigned char *__MapActor_GetActor(int slot); extern void __CutsceneStart(void); extern void __CutsceneEnd(void); extern int __GetPartySize(void); extern void __MessageID(int id); extern void __ActorMessage(int a, | `src/overlays/common/script.c` |
| `OvlFunc_common1_e10` | `` | `StartTransitionEffect` | named | Overlay scripts | extern short L33[] __asm__(".L33"); extern short L22[] __asm__(".L22"); extern short L36[] __asm__(".L36"); extern short L46[] __asm__(".L46"); extern short L24[] __asm__(".L24"); extern void *L37[] __asm__(".L37"); exte | `src/overlays/common/script.c` |
| `OvlFunc_common1_ea0` | `` | `RunDoorSequence` | named | Overlay scripts | so all three linker scripts list the three parts. A cutscene with a short first arm for argument 0 and a long one otherwise. L11 IS DECLARED short[] AND INDEXED, NOT cast-and-offset. Writing the store as *(short *)(L11 + | `src/overlays/common/script.c` |
