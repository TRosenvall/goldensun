 ...
 *
 * THE SCRIPT BLOB NEEDED A RENAME, NOT THE ASM-LABEL EXTENSION. It was `.L7`,
 * and gcc numbers ITS OWN labels .L1, .L2, ... -- so compiling this function
 * with `extern unsigned char L7[] __asm__(".L7")` makes gcc emit a `.L7:` of
 * its own in front of the constant pool, and `ldr r1, .L6+4 / .word .L7` then
 * points at the pool instead of the blob. tryc screens that as ONE differing
 * line (a stray label) and `make compare` fails on the overlay.
 *
 * docs/elevation.md recommends the asm-label extension over renaming, and its
 * examples are `.L23f0` and `.L57fc` -- high numbers gcc's counter never
 * reaches. For a LOW-numbered label the extension is unsafe, and the rename is
 * the only option. Its stated precondition held: exactly one file referenced
 * this symbol across an object boundary, so the rename to gOvlCommon1_3fe4 was
 * a two-line change committed separately with compare green on both sides.
 *
...
extern unsigned char gOvlCommon1_3fe4[];
...
void OvlFunc_common1_172c(struct Actor *a)
{
    ...
    if (__Random() * 0x64 >> 16 <= 9) {
        ...
        if (q != 0) {
            ...
            __Actor_SetScript(q, gOvlCommon1_3fe4);
            __Actor_SetAnim(q, 1);
            __Actor_SetAnim(q, 0);
        }
    }
