extern int gBuffer[];
extern unsigned short ewram_2020000[];
extern unsigned short ewram_2020004[];

void UpdateScreenEdge_H(int blk, int x, int y)
{
    unsigned short *dst;
    int mrow, mx, drow, xx, b;
    unsigned int i;
    int v, kb, o;

    dst = (unsigned short *)(0x6002800 + (blk << 11));
    mrow = ((y / 2) & 0x7f) << 7;
    mx = (x / 2) & 0x7f;
    drow = (y & 0x1e) << 5;
    xx = x & 0x1e;
    b = x & 1;
    for (i = 0; i <= 10; i++) {
        v = gBuffer[mrow + mx];
        kb = ((int)(((unsigned int)v << 20) >> 18) + b) * 2;
        o = (drow + xx + b) * 2;
        *(unsigned short *)((char *)dst + o) = *(unsigned short *)((char *)ewram_2020000 + kb);
        *(unsigned short *)((char *)dst + o + 0x40) = *(unsigned short *)((char *)ewram_2020004 + kb);
        mrow = (mrow + 0x80) & 0x3f80;
        drow = (drow + 0x40) & 0x3c0;
    }
}
