...
extern char *__galloc_iwram(int id, int size);
extern void __LoadItemIcon(int item);
extern void __UploadSpriteGFX(int tile, int count, char *src);
...
int OvlFunc_896_200c260(int item)
{
	...
	if (actor != 0) {
		...
		*(int *)(actor + 0x48) = 0x80 << 7;
		...
		__LoadItemIcon(item);
		__UploadSpriteGFX(*(unsigned char *)(s + 0x1c), 0x80, buf + (0x80 << 3));
		__gfree(0x11);
		__PlaySound(0x53);
