...
static inline int call_via_r8(int (*f)(int, int), int a, int b)
{
    register int (*_f)(int, int) __asm__("r8") = f;
...
void OvlFunc_970_2008f80(void)
{
    ...
    for (i = 0; i != 0xa0; i++) {
        *dst = (unsigned short)(call_via_r8(Func_8000888, L14c8[(acc >> 16) & 0xff], amp) / 256) + add;
        acc += step;
        dst += 2;
    }
