...
# bl displacements are unresolved in a standalone .o; mask those 4-byte slots
bl = set()
...
print("%-8s len %3d  diffs %2d  %s" % (sys.argv[1].split("/")[-1], len(our), len(bad),
      [hex(x) for x in bad][:8]))
PY
