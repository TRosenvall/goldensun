...
@ Steps:
@  1) On first use, allocate a 0x2C4 scratch arena (galloc_iwram) and DMA a copy of
...
@     stack buffer at sp+0x30, then blit each line -- kind 1 via DecompressLZ,
@     kind 3 via DecompressSpriteLZ into a scratch buffer first, otherwise straight
...
@  5) Reserve tiles with UploadSpriteGFX, convert/upload to 0x6010000 + slot*0x20 via
@     the hook at iwram_1e50+0xD4, store the slot in +0x08, free scratch buffers
@     (free) and release the arena (Func_2dd8) if this call created it.
