...
@ in Anim_Summon (rom_d6504.s). 400 frames (0..0x18F), one WaitFrames(1) each.
...
@   AnimStart(0). BuildDraw2DFuncEx(0x2E,7,7,3,2) and BuildDraw2DFuncEx(0x2F,7,7,3,3)
...
@   +0x7780 = 2, +0x7784 = 0x32, StartTask registers Task_BlitAnim.
@
...
@     sp+0x144 + 4i  angle  = atan2(x, y), atan2, 0x10000 = full turn
...
@     projected by .gcc2_compiled., x halved, drawn 16x16 from [state]+0x2400.
...
@     the summon's graphics; Func_d6750; CreateSummonSprite(9, 0x178, 2) spawns NINE
...
@       MatrixRoll(-0x800) / MatrixYaw(-0x1000) fire and 28 ring particles at
@       +0x7400 expand outward at 0x924 apart in angle -- the shockwave.
@   end of frame  Func_cd52c(); +0x7824 = 1; WaitFrames(1).
@
@ TEARDOWN
@   .gcc2_compiled.(0x86) hands back to rom_b5000, then EVERY TARGET'S ORIGINAL x, y
...
@   Func_d67dc; the 9 actors at +0x77D8 are destroyed; StopTask(Task_BlitAnim);
