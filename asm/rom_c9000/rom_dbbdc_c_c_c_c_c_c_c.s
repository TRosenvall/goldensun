...
@ in Anim_Summon (rom_d6504.s). Two frame loops: 220 frames (0..0xDB) of flight
@ and airbursts, then 88 frames (0..0x57) of falling debris and splashes.
@
@ ALONE AMONG THE TWELVE, THIS HANDLER NEVER CALLS .gcc2_compiled.. Every other
...
@   AnimStart(0x2000) -- note the argument; the other handlers pass 0.
...
@     CreateSummonSprite(9, 0x172, 1) spawns nine actors.
@   LoadVFXFile unpacks asset 0x6A into [iwram_1eec] and 0x73 into [iwram_1ef4];
@     asset 0xA0's first 0x80 bytes go to palette RAM.
@   sp+0x30 = GetFile(0xD2) -- THE MOTION PATH, read below.
@   BuildDraw2DFuncEx(0x2E,7,7,3,2) and BuildDraw2DFuncEx(0x2F,7,7,3,3), stored as the pair at
@     sp+0x3C for (i & 1) indexing.
@   iwram_1ce0+0x10 = 0xF0; WaitFrames(1); _Func_c08ec(1, 0x3B, 0).
@   +0x7790 = 0, +0x7794 = 4, +0x7798 = -1, +0x779C = 0, and StartTask registers
...
@   own random orientation: InitMatrixStack loads identity, MatrixRoll / MatrixPitch /
@   MatrixYaw rotate by three random 16-bit angles, and MatrixStore stores the
@   result as 0x30 bytes at ewram_13800 + 0x30*n. Loop one then replays them
@   with MatrixLoad, so the debris tumbles without any per-frame trig.
@
...
@       matrix, projected with .gcc2_compiled. and drawn at 5x10 from Data_ede48[4];
...
@   end of frame  +0x7824 = 1; WaitFrames(1). A or B held leaves loop one.
@
@ BETWEEN THE LOOPS
@   StopTask(Func_c90e4); iwram_1ad0+4 restored; Func_d67dc; the nine actors
@   are destroyed HERE rather than at teardown; REG_BG2PA = 0x80;
@   REG_DISPCNT = 0x7741; LoadVFXFile unpacks asset 0xB4 into ewram_10000, which
