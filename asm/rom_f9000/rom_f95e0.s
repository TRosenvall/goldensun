...
@ four -- onto the pattern stack at track+0x44 and jumps like ply_goto. THE
@ STACK IS THREE DEEP: a level of 3 or more falls through to ply_fine and ends
...
@ +0x1E the scale m4aMPlayTempoControl sets, and +0x20 the effective rate the sequencer
...
@ also the running-status threshold in MP2KPlayerMain.
...
@ walks every track:
@
@   * expired channels are released through ClearChain and freed with
@     Clear64byte; the note-off mask 0xC7 and the release bit 0x40 are the same
@     ones ply_fine uses
...
@
@ FadeOutBody does the per-track volume and pitch, TrkVolPitSet the envelopes.
